...
fn extension(path_hint: &str) -> Option<&str> {
    ...
    base.rsplit_once('.').map(|(_, ext)| ext).filter(|ext| !ext.is_empty() && *ext != base)
}
...
pub fn classify(bytes: &[u8], path_hint: &str) -> Classification {
    ...
    let confidently_binary =
        sniff.has_nul || !sniff.is_valid_utf8 || sniff.control_ratio > 0.30 || sniff.entropy_bits > 7.6;
    ...
    let (language, kind, confidence) = match (fingerprint, shebang, ext_hint) {
        (Some((lang, kind, score)), shebang, ext) => {
            let shebang_bonus = if shebang == Some(lang) { WEIGHT_SHEBANG } else { 0.0 };
        ...
        (None, Some(lang), _) => (Some(lang), ContentKind::Code, WEIGHT_SHEBANG + WEIGHT_METADATA_PRIOR),
...
fn vendor_probability(path_hint: &str) -> f32 {
    const VENDOR_SEGMENTS: &[&str] = &["vendor", "node_modules", "third_party", "third-party", ".venv"];
    if path_hint
        .split('/')
